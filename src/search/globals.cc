...
static const string PRE_FILE_VERSION = "3.FOND";
static const string PRE_FILE_VERSION_ALTERNATIVE = "3";
...
static vector<vector<pair<int, int> > > g_invariant_groups;
...
void read_and_verify_version(istream &in) {
    ...
    if (version != PRE_FILE_VERSION && version != PRE_FILE_VERSION_ALTERNATIVE) {
...
void read_everything(istream &in) {
	...
	read_operators(in);
    read_axioms(in);
    // Right now working on SAS after translation, without preprocessing, skipping SG, DTG, and CG.
...
void write_and_verify_version(ostream &out) {
	out << "begin_version" << endl << 3 << endl << "end_version" << endl;
}

void write_metric(ostream &out) {
	out << "begin_metric" << endl << g_use_metric << endl << "end_metric" << endl;
}

void write_variables(ostream &out) {
	out << g_variable_name.size() << endl;
    for (size_t i = 0; i < g_variable_name.size(); ++i) {
    	out << "begin_variable" << endl;
    	out << g_variable_name[i] << endl;
    	out << g_axiom_layers[i] << endl;
    	vector<string>& fact_names = g_fact_names[i];
    	out << fact_names.size() << endl;
        for (size_t j = 0; j < fact_names.size(); ++j)
        	out << fact_names[j] << endl;
    	out << "end_variable" << endl;
    }
}

void write_mutexes(ostream &out) {
	out << g_invariant_groups.size() << endl;
    for (size_t i = 0; i < g_invariant_groups.size(); ++i) {
    	out << "begin_mutex_group" << endl;
        vector<pair<int, int> >& invariant_group = g_invariant_groups[i];
		out << invariant_group.size() << endl;
        for (size_t j = 0; j < invariant_group.size(); ++j) {
        	pair<int, int> &fact = invariant_group[j];
			out << fact.first << " " << fact.second << endl;
        }
    	out << "end_mutex_group" << endl;
    }
}

void write_goal(ostream &out) {
	out << "begin_goal" << endl;
	out << g_goal.size() << endl;
    for (size_t i = 0; i < g_goal.size(); ++i) {
    	pair<int, int> &fact = g_goal[i];
    	out << fact.first << " " << fact.second << endl;
    }
	out << "end_goal" << endl;
}

void write_operators(ostream &out) {
	out << g_operators.size() << endl;

    for (size_t i = 0; i < g_operators.size(); ++i)
    	out << g_operators[i];
}

void write_axioms(ostream &out) {
	out << g_axioms.size() << endl;

    for (size_t i = 0; i < g_axioms.size(); ++i)
    	out << g_axioms[i];
}

void write_everything(ostream &out) {
	write_and_verify_version(out);
	write_metric(out);
	write_variables(out);
	write_mutexes(out);
	out << "begin_state" << endl;
    for (size_t i = 0; i < g_variable_domain.size(); ++i) {
    	out << g_initial_state_data[i] << endl;
    }
	out << "end_state" << endl;
