...
void LandmarkCutHeuristic::build_relaxed_operator(const OperatorProxy &op) {
	...
	// We relax the nondeterminism here
	for (EffectsProxy outcome : op.get_nondet_effects()) {
		for (size_t i = 0; i < outcome.size(); ++i) {
			effects.push_back(get_proposition(outcome[i].get_fact()));
			debug(1, _ARGS);
		}
		printValue(effects.size(), "effects.size", _ARGS);
		add_relaxed_operator(move(precondition), move(effects), op.get_id(), op.get_cost());
	}
}
