...
class EffectProxy {
    ...
    int outcome;
public:
    EffectProxy(const AbstractTask &task, int op_index, int outcome, int eff_index, bool is_axiom)
        : task(&task), op_index(op_index), eff_index(eff_index), is_axiom(is_axiom), outcome(outcome) {}
    ~EffectProxy() = default;
    ...
    FactProxy get_fact() const {
        std::pair<int, int> fact =
            task->get_operator_effect(op_index,  outcome ,eff_index, is_axiom);
        return FactProxy(*task, fact.first, fact.second);
    }
};
...
public:
    ...
    EffectsProxy(const AbstractTask &task, int op_index, bool is_axiom, int outcome)
        : task(&task), op_index(op_index), is_axiom(is_axiom), outcome(outcome) {}
    ~EffectsProxy() = default;

    std::size_t size() const {
        return task->get_num_operator_effects(op_index, outcome, is_axiom);
    }

    EffectProxy operator[](std::size_t eff_index) const {
        assert(eff_index < size());
        return EffectProxy(*task, op_index, outcome,  eff_index, is_axiom);
    }
};
...
public:
	...
	std::vector<EffectsProxy> get_nondet_effects() const {
		std::vector<EffectsProxy> outcomes;
		int num_of_outcomes = task->get_num_operator_outcomes(index, is_an_axiom);
		for (int i = 0; i < num_of_outcomes; ++i) {
			EffectsProxy e(*task, index, is_an_axiom, i);
			outcomes.push_back(e);
		}	
		return outcomes;
    }
...
public:
    ...
    State get_successors(OperatorProxy op) const {
		...
		for (auto outcome: op.get_nondet_effects()) {
			for (EffectProxy effect : outcome) {
