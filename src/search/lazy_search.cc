...
SearchStatus LazySearch::step() {
    ...
    /*
      When using orbit search, current_state is the canonical representative of the state
      obtained by applying current_operator in the predecessor state.
      That can affect path dependent heuristics.
      real_current_state is the actual state obtained by applying current_operator in the predecessor state.

     */
	GlobalState real_current_state = current_state;
	if (current_predecessor_id != StateID::no_state) {
		GlobalState current_predecessor = g_state_registry->lookup_state(current_predecessor_id);
		StateRegistry tmp_registry;
		StateRegistry *successor_registry = do_orbit_search? &tmp_registry : g_state_registry;
		real_current_state = successor_registry->get_successor_state(current_predecessor, *current_operator);
		current_state = real_current_state;

		if (do_orbit_search) {
			current_state = g_state_registry->get_canonical_representative(real_current_state);
		}
	}
