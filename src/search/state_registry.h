...
public:
    ...
    */
    ...
    /*
      Returns the states that result from applying op to predecessor and
      registers it if this was not done before. This is an expensive operation
      as it includes duplicate checking.
      
      Note: In FOND planning the application of an operator leads to a set of successor states
      
    */
    void get_successor_states(const GlobalState &predecessor, const GlobalOperator &op, std::vector<GlobalState>& successors);
    
    /*
      Returns the canonical of the states that result from applying op to predecessor and
      registers it if this was not done before. This is an expensive operation
      as it includes duplicate checking.
    ...
    */
    void get_canonical_successor_states(const GlobalState &predecessor, const GlobalOperator &op, std::vector<GlobalState>& successors);
    
    /*
      Creates the canonical representative of the given GlobalState (which can be
      ...
      duplicate checking.
    */
    GlobalState get_canonical_representative(const GlobalState &state);

    /*
      Creates the permutation of the given GlobalState (which can be registered
      ...
      checking.
    */
    GlobalState get_state_permutation(const GlobalState &state, Permutation &permutation);

    /*
      Adds a state possibly from a different registry. Registers and returns
      the state if this was not done before. This is an expensive operation
      as it includes duplicate checking.
    */
    GlobalState add_state(const GlobalState &state);
