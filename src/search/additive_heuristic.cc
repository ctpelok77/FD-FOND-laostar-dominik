...
//		dump_unaries(triggered_operators);
        for (size_t i = 0; i < triggered_operators.size(); ++i) {
            UnaryOperator *unary_op = triggered_operators[i];
//            std::cout << "Triggered operator " << unary_op->operator_no << " with effect " << unary_op->effect->id << std::endl;
...
int AdditiveHeuristic::compute_heuristic(const GlobalState &global_state) {
    ...
    // In the current version of LAO* that we use,
    // mark_preferred_operators can cause problems, possibly due to
    // operator indices. For now, we do not use preferred operators
    // anyway, but we should have a closer look what happens there
    // eventually.
...
void AdditiveHeuristic::dump_unaries(const std::vector<UnaryOperator *> vec)
{
	//for (auto& o : vec) {
	//	std::cout <<"op_nr "<< std::to_string(o->operator_no) << endl;			
	//}	
}
