...
pair<int, int> RootTask::get_operator_precondition(int op_index, int fact_index, bool is_axiom) const {
...
int RootTask::get_num_operator_effects(int op_index, int outcome,  bool is_axiom) const {
    return get_operator_or_axiom(op_index, is_axiom).get_non_deterministic_effects()[outcome].size();
}

int RootTask::get_num_operator_outcomes(int op_index,  bool is_axiom) const {
    return get_operator_or_axiom(op_index, is_axiom).get_non_deterministic_effects().size();
}

// virtual int get_num_operator_effect_conditions(int op_index, int eff_index, bool is_axiom) const override;

int RootTask::get_num_operator_effect_conditions(int op_index, int eff_index, bool is_axiom) const {
	return -1;
...
pair<int, int> RootTask::get_operator_effect(int op_index, int outcome, int eff_index, bool is_axiom) const {
    const GlobalEffect &effect = get_operator_or_axiom(op_index, is_axiom).get_non_deterministic_effects()[outcome][eff_index];
